...
// use gazpacho_fixtures::{self as fixtures, Spec, SyntheticVideo, Timing};
...
//     for video in fixtures.spec_backed() {
//         let spec = &video.spec;
...
//         }));
//     }
//     for video in [fixtures.baseline()?, fixtures.expect_spec("h264_420p_g12_vfr.mp4")?] {
//         trials.push(trial(
...
//         ));
//     }
//     let baseline = fixtures.baseline()?;
//     trials.push(trial(
...
//     ] {
//         let video = fixtures.expect_spec(name)?;
//         trials.push(trial("bframe_reordering_is_invisible", video, move || {
//             bframe_reordering_is_invisible(video)
//         }));
//     }
//     // Clips that failed to generate (missing encoder) are surfaced as ignored
...
// fn metadata_matches_spec(video: &SyntheticVideo, spec: &Spec) -> eyre::Result<()> {
//     let name = &video.name;
...
// fn every_frame_recovers_its_index(video: &SyntheticVideo, spec: &Spec) -> eyre::Result<()> {
...
//     let mut reader = reader();
//     let t = media_time(video.spec.timestamp_of(7));

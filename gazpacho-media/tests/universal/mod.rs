...
use crate::{assert_frames_eq, fixture_resolution, probed_timestamps, reader};
...
pub(crate) fn metadata_loads(video: &TestVideo) -> eyre::Result<()> {
...
pub(crate) fn fast_load_agrees_with_full_decode(video: &TestVideo) -> eyre::Result<()> {
...
#[track_caller]
pub(crate) fn assert_agree(label: &str, fast: &MediaMetadata, slow: &MediaMetadata) {
    assert_eq!(
        fast.video.len(),
        slow.video.len(),
...
pub(crate) fn extent_is_self_consistent(video: &TestVideo) -> eyre::Result<()> {
...
/// not.
pub(crate) fn sequential_read_matches_reference_decode(video: &TestVideo) -> eyre::Result<()> {
    let meta = MediaMetadata::load(&video.path)?;
    let stream = meta
        .video
        .first()
...
pub(crate) fn random_access_matches_sequential(video: &TestVideo) -> eyre::Result<()> {
    let meta = MediaMetadata::load(&video.path)?;
    let stream = meta
        .video
        .first()
...
pub(crate) fn frame_hash(frame: &Frame) -> u64 {
    use std::hash::Hash;
...
pub(crate) fn out_of_extent_is_an_error(video: &TestVideo) -> eyre::Result<()> {
    let mut reader = reader();
    let extent = reader.extent(&video.path)?;
